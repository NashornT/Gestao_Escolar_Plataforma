...
def create_app():
    ...
    global turma_table, disciplina_table, aluno_table, nota_table, alunos_turma_table, professor_table, professores_turmas_disciplinas_table, anuncio_table # <-- ADICIONE AQUI
    with app.app_context():
        try:
            ...
            professor_table = Table('professores', db.metadata, autoload_with=academic_engine)
            professores_turmas_disciplinas_table = Table('professores_turmas_disciplinas', db.metadata, autoload_with=academic_engine)
            anuncio_table = Table('anuncios', db.metadata, autoload_with=academic_engine)
            material_aula_table = Table('materiais_aula', db.metadata, autoload_with=academic_engine)
